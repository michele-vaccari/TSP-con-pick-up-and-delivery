...
class LargeNeighborhoodSearchBenchmark(Observer):
    ...
    def benchmark(self):
        ...
        column_index += 1
        sheet.cell(row=row_index, column = column_index).value = "Tasso di distruzione"
        column_index += 1
        sheet.cell(row=row_index, column = column_index).value = "k massimo"
        column_index += 1
        sheet.cell(row=row_index, column = column_index).value = "Massimo numero di peggioramenti"
        ...
        for requests in range(self._requests_benchmark_start, self._requests_benchmark_end + 1):
            ...
            largeNeighborhoodSearch = LargeNeighborhoodSearch(instance, self._greedy_best_solutions[requests][1])
            largeNeighborhoodSearch.attach(self)
            solution = largeNeighborhoodSearch.calculate_solution()
            largeNeighborhoodSearch.detach(self)
            ...
            sheet.cell(row=row_index, column=5).value = largeNeighborhoodSearch.feasible_solutions_counter
            sheet.cell(row=row_index, column=6).value = largeNeighborhoodSearch.destruction_rate
            sheet.cell(row=row_index, column=7).value = largeNeighborhoodSearch.k_max
            sheet.cell(row=row_index, column=8).value = largeNeighborhoodSearch.max_number_of_worsening
