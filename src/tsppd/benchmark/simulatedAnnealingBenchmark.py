...
class SimulatedAnnealingBenchmark(Observer):
    ...
    def benchmark(self):
        ...
        column_index += 1
        sheet.cell(row=row_index, column = column_index).value = "Temperatura iniziale"
        column_index += 1
        sheet.cell(row=row_index, column = column_index).value = "Temperatura finale"
        column_index += 1
        sheet.cell(row=row_index, column = column_index).value = "Tasso di raffreddamento"
        column_index += 1
        sheet.cell(row=row_index, column = column_index).value = "Iterazioni per temperatura"
        ...
        for requests in range(self._requests_benchmark_start, self._requests_benchmark_end + 1):
            ...
            simulatedAnnealing = SimulatedAnnealing(instance, self._greedy_best_solutions[requests][1])
            simulatedAnnealing.attach(self)
            solution = simulatedAnnealing.calculate_solution()
            simulatedAnnealing.detach(self)
            ...
            sheet.cell(row=row_index, column=5).value = simulatedAnnealing.feasible_solutions_counter
            sheet.cell(row=row_index, column=6).value = simulatedAnnealing.initial_temperature
            sheet.cell(row=row_index, column=7).value = simulatedAnnealing.final_temperature
            sheet.cell(row=row_index, column=8).value = simulatedAnnealing.cooling_rate
            sheet.cell(row=row_index, column=9).value = simulatedAnnealing.iterations_for_temperature
