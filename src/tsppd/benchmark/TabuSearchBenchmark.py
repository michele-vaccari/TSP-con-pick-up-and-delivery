...
class TabuSearchBenchmark(Observer):
    ...
    def benchmark(self):
        ...
        for requests in range(self._requests_benchmark_start, self._requests_benchmark_end + 1):
            ...
            tabuSearch = TabuSearch(instance, self._greedy_best_solutions[requests][1])
            tabuSearch.attach(self)
            solution = tabuSearch.calculate_solution()
            tabuSearch.detach(self)
            ...
            sheet.cell(row=row_index, column=5).value = tabuSearch.feasible_solutions_counter
