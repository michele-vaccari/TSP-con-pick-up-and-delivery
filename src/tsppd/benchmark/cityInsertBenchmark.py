...
class CityInsertBenchmark(Observer):
    ...
    def benchmark(self):
        ...
        for requests in range(self._requests_benchmark_start, self._requests_benchmark_end + 1):
            ...
            cityInsert = CityInsert(instance, self._greedy_best_solutions[requests][1])
            cityInsert.attach(self)
            solution = cityInsert.calculate_solution()
            cityInsert.detach(self)
            ...
            sheet.cell(row=row_index, column=5).value = cityInsert.feasible_solutions_counter
