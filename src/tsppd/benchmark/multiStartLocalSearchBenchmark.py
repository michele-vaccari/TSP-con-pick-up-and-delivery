...
class MultiStartLocalSearchBenchmark(Observer):
    ...
    def benchmark(self):
        ...
        for requests in range(self._requests_benchmark_start, self._requests_benchmark_end + 1):
            ...
            multiStartLocalSearch.attach(self)
            solution = multiStartLocalSearch.calculate_solution()
            multiStartLocalSearch.detach(self)
            ...
            sheet.cell(row=row_index, column=5).value = multiStartLocalSearch.feasible_solutions_counter
            sheet.cell(row=row_index, column=6).value = multiStartLocalSearch.max_iterations_number
